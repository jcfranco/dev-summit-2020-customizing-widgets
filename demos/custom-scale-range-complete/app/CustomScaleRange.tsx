...
class ScaleRangeSlider extends declared(HandleOwnerMixin(Widget)) {
  ...
  protected renderScaleMenu({ map, min, max, type }: ScaleMenuProps) {
  ...
  protected renderScaleMenuToggle(type: ScaleType, label: string) {
  ...
  protected renderScaleMenuItem(props: ScaleMenuItemProps) {
    const {
      scaleValue,
      scaleLabel,
      valueVisible,
  ...
  protected renderCurrentScaleIndicator() {
    const {
      _slider,
      view,
      viewModel: { scaleRanges }
